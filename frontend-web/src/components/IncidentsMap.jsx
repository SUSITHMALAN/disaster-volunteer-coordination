...
export default function IncidentsMap({ onSelectIncident }) {
  ...
  useEffect(() => {
    ...
    const CARTO_KEY = import.meta.env.VITE_CARTO_API_KEY || "cb1_3wna_1_dd1faf8169540033ea0e561d";

    // CARTO Dark Matter tiles using new CARTO rastertiles service with key parameter
    const tileUrl = `https://basemaps.cartocdn.com/rastertiles/dark_all/{z}/{x}/{y}{r}.png?key=${CARTO_KEY}`;
  ...
  // Place / update markers whenever incidents or filter changes
  useEffect(() => {
    ...
    <div className="incidents-map-card">
      <div className="incidents-map-header">
        <div className="incidents-map-header__left">
          <h2 className="incidents-map-title">🗺️ Live Incident Risk Map</h2>
          <p className="incidents-map-subtitle">
            Real-time geospatial monitoring · CARTO Dark Matter · Severity-pinned incidents
          </p>
        </div>

        <div className="incidents-map-stats">
          <span
            className="incidents-map-stat incidents-map-stat--critical"
            title="Critical incidents requiring urgent deployment"
          >
            🚨 {criticalCount} Critical
          </span>
          <span
            className="incidents-map-stat incidents-map-stat--high"
            title="High severity incidents"
          >
            ⚠️ {highCount} High
          </span>
          <span
            className="incidents-map-stat incidents-map-stat--total"
            title="Total active disaster alerts"
          >
        ...
        <div className="incidents-map-filters">
          ...
          {["All", "Critical", "High", "Medium", "Low"].map((sev) => (
            <button
              key={sev}
              type="button"
              className={`incidents-map-filter-btn ${
                severityFilter === sev ? "incidents-map-filter-btn--active" : ""
              } incidents-map-filter-btn--${sev.toLowerCase()}`}
      ...
      <div className="incidents-map-layout">
        <div className="incidents-map-canvas-wrapper">
          ...
          <div ref={mapContainerRef} className="incidents-map-canvas" />
        </div>
        ...
        <div className="incidents-map-sidebar">
          ...
          <div className="incidents-map-sidebar__list">
            {filteredIncidents.map((inc) => (
              <div
                key={inc.id}
                className={`incidents-map-item incidents-map-item--${inc.severity.toLowerCase()} ${
                  selectedIncident?.id === inc.id ? "incidents-map-item--selected" : ""
                }`}
              ...
              >
                <div className="incidents-map-item__top">
                  ...
                  <span
                    className={`incidents-map-item__badge incidents-map-item__badge--${inc.severity.toLowerCase()}`}
                  >
                    {inc.severity}
                  </span>
                </div>
